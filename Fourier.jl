...
The mathematical expression for the fourier transform is...daunting to say the least.
...
A transform takes a function as an input and spits out another function as its output.


One of the fundamental facts of the universe:
...
html"""
<img width="200px" src="https://upload.wikimedia.org/wikipedia/commons/thumb/d/df/Fourier2_-_restoration1.jpg/800px-Fourier2_-_restoration1.jpg" />
<p><em>Jean-Baptiste Joseph Fourier</em></p>
"""
...
md"""
#### Why is there the $2\pi$ term in the sine wave?

This is very common in signal processing. The $2\pi$ is to make the $f$ represent cycles per second (assumin $t$ is in seconds).

For example, if $f = 2$, then there will be exactly $2$ oscillations between $t = 0$ and $t = 1$.

"""

# ╔═╡ 38723231-123e-4207-8356-20fed4c147c4
begin
	many_ys = sin.(xs) + sin.(2*xs) + sin.(3*xs) + sin.(4*xs) 
	plot(xs, many_ys, title="combination of many sine waves of different 
freqs", label="sum")
	plot!(xs, sin.(xs), alpha=0.5, label="sin(x)")
	plot!(xs, sin.(2*xs), alpha=0.5, label="sin(2x)")
	plot!(xs, sin.(3*xs), alpha=0.5, label="sin(3x)")
	plot!(xs, sin.(4*xs), alpha=0.5, label="sin(4x)")
end

# ╔═╡ 441d7ad0-114a-4fe9-a009-cbc6f32ce308
md"""
The Fourier transform decomposes a waveform into sinusoids.

In general, waveforms are not made up of discrete number of frequencies, but rather a continuous range of frequencies.

To dig deeper into Fourier Transform, we first learn about the *Fourier Series*.
...
A Fourier Series, with period $T$, is an infinite sum of sinusoidal functions (cosine and sine), each with a frequency that is an integer multiple of $1/T$ (the inverse of the fundamental period).

$g(t) = \sum_{m = 0}^{\infty} a_m \cos (\dfrac{2\pi m t}{T}) + \sum_{n = 1}^{\infty} b_n \sin (\dfrac{2\pi n t}{T})$

> For an arbritary periodic function $f(t)$ - how closely can we approximate this function with simple sinusoids, each with a period some integer multiple of the fundamental period?

We can approximate $f(t)$ exactly whenever $f(t)$ is continuous and *smooth*. IRL, all functions are continuous and smooth, so practically all periodic functions can be *exactly* represented by Fourier Series.

#### But how do we determine the cofficients $a_m$, $b_n$?

Let's start by looking at the first term! 

$g(t) = a_0$

The best value for $a_0$ will be the avearge of $f(t)$ over its period $T$. So, since $f(t)$ is continuous, this can be represented with an integral
...
md"""
Let's take another term, say the $b_1$ term.

$g(t) = a_0 + \boxed{b_1} \sin \dfrac{2 \pi t}{T}$

What is the optimal value for $b_1$? Intuitively, we want to know how correlated $f$ is with $\sin \dfrac{2 \pi t}{T}$. In a sense, want to correlate the function $f$ and the basis sine function,

$b_1 = \dfrac{2}{T} \int_{0}^{T} f(t) \sin \dfrac{2 \pi t}{T}dt = \dfrac{2}{\pi}$

"""

# ╔═╡ 34c392fd-c52a-42e4-bbd7-6d79ca641291
begin
	ts = [x for x in range(-1, 1, 1000)]
	y = (sign.(sin.(2π * ts)) + fill(1, length(ts)))/2
	plot(ts, y, label="", xlabel="Time (s)", ylabel="Amplitude")
	a0 = fill(0.5, length(ts))
	plot!(ts, a0, label="one term", alpha=0.5)
	a0plusb1 =  [0.5 + ((2/π)*sin(2*π*1*t/1)) for t in ts]
	plot!(ts, a0plusb1, label="two terms", alpha=0.5)
end
...
$a_m = \dfrac{2}{T} \int_{0}^{T} f(t) \cos \dfrac{2 \pi m t}{T}dt$
$b_n = \dfrac{2}{T} \int_{0}^{T} f(t) \sin \dfrac{2 \pi n t}{T}dt$

All of these represent the correlation of the function $f(t)$ with the basis sine and cosine functions.

For our square wave, the coeffecients come out to be:
$a_m = 0$ for all m, $b_n = \dfrac{2}{\pi n}$ for n odd and $0$ otherwise.
...
begin
	plot(ts, y, label="", xlabel="Time (s)", ylabel="Amplitude")
	g(t, count) = 0.5 + sum([(2 / (π * n)) * sin(2 * π * n * t / 1) for n in range(1, count) if n % 2 != 0])
...
@bind k Slider(1:1000, default=4)
...
@bind saw_count Slider(1:30, default=4) 
...
html"""
<iframe width="560" height="315" src="https://www.youtube.com/embed/spUNpyF58BY?si=ox1YDzrihVcZAd87" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
"""

# ╔═╡ def53162-c99a-4841-982b-ad4144d3033e
html"""
<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/r6sGWTCMz2k?si=MSjPpMcj5k7m-7SX" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
