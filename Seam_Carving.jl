...
# ╔═╡ f2cd5db3-08c8-45f1-a252-e9156695f7f2
using Images, ImageFiltering, PlutoUI

# ╔═╡ 26232c42-ba3f-11ef-0767-6ff4cb6d394a
md"""
# Seam Carving

Seam carving (or liquid rescaling or *content-aware scaling*) is an algorithm for content-aware image resizing.
...
We can use Sobel filters to detect edges and then proceed to remove the uninteresting parts.
"""

# ╔═╡ ba9d2868-8db5-4661-9965-3c26d561ebb7
begin
	large_image = load("cat_sitting.jpg")
	image = imresize(large_image, ratio = 0.5)
end

# ╔═╡ be0377e8-72d9-442b-9cc5-3782c5a45137
Sy, Sx = Kernel.sobel()

# ╔═╡ be723429-06d0-4498-9e9f-9dd82783f60d
Sx

# ╔═╡ 02525fad-1917-44a2-bbc3-e647b8f0fd14
...
# ╔═╡ e259e3bf-d53f-49e7-a85f-80c3f562ff10
begin
	∇x = imfilter(image, reflect(Sx))
	∇y = imfilter(image, reflect(Sy))
	∇x, ∇y
end

# ╔═╡ 51e6c9c7-2e51-4c8d-a54e-d80de320c4ff
edgy_image = sqrt.(Gray.(∇x).^2 + Gray.(∇y).^2)

# ╔═╡ 796837f1-e6bd-4173-b75b-8f01febc99ef
md"""
### Core idea of Seam Carving
...
We start by writing a `least_edgy` function which given a matrix of energies, returns a matrix of minimum possible energy starting from that pixel going up to a pixel in the bottom most row.

"""

# ╔═╡ 87f06322-e033-45eb-b4d3-5019946bb9d3
# The input is the edgy image
function least_edgy(E)
	least_E = zeros(size(E))
	dirs = zeros(Int, size(E))
	least_E[end, :] .= E[end, :] # the min energy on the last row is the energy of 									the last row itself
	m, n = size(E)
...
# ╔═╡ 95a508bf-06b0-4aab-aabe-2e477dd32562
# The bright areas are screaming "AVOID ME!!!"
least_e, dirs = least_edgy(edgy_image)

# ╔═╡ fe410994-a5cb-4609-9e2b-0c7b9f8f5505
show_colored_array(least_e)

# ╔═╡ 37564a1d-cf0c-4b39-b918-5018682758cc
md"""
### Removing seams
"""

# ╔═╡ 5ba442ee-3eee-4576-9433-747ad1aeaca3
...
# ╔═╡ 6e56f79d-7c0f-4e10-b956-011135778974
...
# ╔═╡ 9425fcf6-70b9-43c1-a6cf-28afa25e9257
@bind start_column Slider(1:size(image, 2))

# ╔═╡ c562a19a-7aac-408e-903d-384dd4673e3c
path = get_seam_at(dirs, start_column)

# ╔═╡ 59361ab4-9c85-473c-a25e-6813a7091502
mark_path(image, path)

# ╔═╡ 913f5f74-54e3-4427-8586-fbc23fdfebff
...
# ╔═╡ a705d412-22d7-4d4f-8c7d-9fe3600dee81
...
# ╔═╡ a0813dcb-7fa4-4438-a779-3762f70e2d54
n_examples = min(200, size(image, 2))

# ╔═╡ a1b65c87-2a82-454c-8ed8-c11b621e7ddb
carved, marked_carved = shrink_n(image, n_examples)


# ╔═╡ 122bee11-41e1-49b8-ab0f-071026b9efcb
...
# ╔═╡ a91d410c-d92e-4204-9e93-a74857fc4622
@bind n Slider(1:length(carved))

# ╔═╡ 1e57bf8c-8cd4-4e1a-a8f9-dc4425975d71
hbox(image, marked_carved[n], sy=size(image))
